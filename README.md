...
3. Display results on an **Interactive Map**, using the [OpenStreetMap API](https://www.openstreetmap.org/) to download tiles and generate **Heatmaps** with a CPU-based algorithm.  
   Geolocated measures are displayed in Unity through prefab generation and camera movement handlers.
...
In this context, Unity can be seen as an integrated development environment for interactive applications, much like Android Studio for Android or Xcode for Apple platforms. A natural next step for the software would be to evolve into a web-based product.
...
- **Query Engine**
  ...
  - `Query Services` convert customer requests into an executable query request.  
...
- The current ETL suffers from a single-threaded, per-line buffer for staging data. We should reduce processing time with potential solutions:
  - CSV files should be loaded as-is into a staging table (with DuckDB for example), and the transformations should be delegated to the database before the merge operation.
...
- History and prediction files can evolve over time, so we will have to handle:
  - Automatic download
  - Asynchronous data pipelines with initial and delta jobs
  - Cron jobs for derived tables, triggered according to their level (for example: hour, day, month, year)
