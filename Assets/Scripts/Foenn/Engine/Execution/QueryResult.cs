...
namespace Assets.Scripts.Foenn.Engine.Execution
{
    public class QueryResult
    {
        ...
        private bool TryParseMetric(object value, out Metric parsedValue)
        {
            ...
            if (match.Success)
            {
                parsedValue = new Metric(
                    System.Enum.Parse<WeatherHistoryMetricKey>(match.Groups[1].Value),
                    System.Enum.Parse<AggregationKey>(match.Groups[0].Value)
                );
                return true;
            }
            parsedValue = null;
        ...
        private void AddDimensionParser(Attribute attribute) {
            ...
            {
                columnParser.Add((Row row, object value) => AddTimeDimension(row, attribute, value));
            }
            else if (attribute.key.Equals(WeatherHistoryAttributeKey.NUM_POSTE))
            {
                columnParser.Add((Row row, object value) => AddGeoDimension(row, attribute, value));
            }
            else
            {
                columnParser.Add((Row row, object value) => AddAttributeValue(row, attribute, value));
            }
        }
        ...
        private void AddTimeDimension(Row row, Attribute attribute, object value)
        {
            row.time = TimeDimension.AAAAMMJJHH((string)value);
        }

        private void AddGeoDimension(Row row, Attribute attribute, object value)
        {
            row.geo = new GeoDimension() { numPost = value.ToString() };
        }

        private void AddAttributeValue(Row row, Attribute attribute, object value) { 
