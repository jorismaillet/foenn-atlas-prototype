...
namespace Assets.Editor.Tests.Models
{
    public class ActivityTest
    {
        ...
        public void TestActivityCreation()
        {
            ...
            var tempCondition = new FloatRangeCondition(WeatherCoreFact.temperature, 20, 30);
            var rainCondition = new FloatRangeCondition(WeatherCoreFact.rain, 0, 0);
            var activity = new Activity("Beach", hourCondition, tempCondition, rainCondition);
            var row = new Row();
            row.values[TimeDimension.hour] = 15;
            row.values[WeatherCoreFact.rain] = 0;
            row.values[WeatherCoreFact.temperature] = 25F;
